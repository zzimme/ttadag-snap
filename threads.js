...
/*
        ...
        cbcolorURL: {
            type: 'reporter',
            category: 'sensing',
            spec: '색깔 : 주소 : %s 몇번째 : %s  빨강 %s 초록 %s 파랑 %s',
            defaults: ['3', '1', '255','255','255']
        },  
*/
Process.prototype.cbrainbowURL = function (url) {
    var response;
    if (!this.httpRequest) {
        this.httpRequest = new XMLHttpRequest();
        this.httpRequest.open("GET", 'http://192.168.0.'+url+':42001/command:rainbow', true);
        this.httpRequest.send(null);
    } else if (this.httpRequest.readyState === 4) {
        response = this.httpRequest.responseText;
        this.httpRequest = null;
...
Process.prototype.cbrotateURL = function (url) {
    var response;
    if (!this.httpRequest) {
        this.httpRequest = new XMLHttpRequest();
        this.httpRequest.open("GET", 'http://192.168.0.'+url+':42001/command:rotate', true);
        this.httpRequest.send(null);
    } else if (this.httpRequest.readyState === 4) {
        response = this.httpRequest.responseText;
        this.httpRequest = null;
...
Process.prototype.cbcolorURL = function (url, pos, red, green, blue) {
    var response;
    if (!this.httpRequest) {
        this.httpRequest = new XMLHttpRequest();
        this.httpRequest.open("GET", 'http://192.168.0.'+url+':42001/command:color:'+pos+':'+red+':'+green+':'+blue, true);
        //this.httpRequest.open("GET", 'http://192.168.0.16:42001/command:'+url, true);
        this.httpRequest.send(null);
    } else if (this.httpRequest.readyState === 4) {
        response = this.httpRequest.responseText;
        this.httpRequest = null;
